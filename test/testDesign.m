...
planner = planner.addParallelJawGripperConstraints();
...
r = {};
for i = 1:planner.n_e/2-1
	r{i} = zeros(2,t);
	r{i}(1,:) = planner.vars.p_colloc.value(1,i,1:t);
	r{i}(2,:) = planner.vars.p_colloc.value(2,i,1:t);
	scatter(r{i}(1,:),r{i}(2,:),'g');
	hold on;
end

for i = planner.n_e/2+1:planner.n_e-1
	r{i} = zeros(2,t);
	r{i}(1,:) = planner.vars.p_colloc.value(1,i,1:t);
	r{i}(2,:) = planner.vars.p_colloc.value(2,i,1:t);
	scatter(r{i}(1,:),r{i}(2,:),'k');
	hold on;
end
